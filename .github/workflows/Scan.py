...
try:
    ...
    send_notification(f"Scan *start* at *{start_time:%Y-%m-%d %H:%M}*", parse_mode='Markdown', disable_notification=True)
    ...
    for asn in unique_asns:
        send_to_telegram(os.path.join(output_path, f"ASN{asn}.txt"), disable_notification=True)

    scan_and_send_files(best_cf_url, "BestCF", additional_text="`bestcf.onecf.eu.org`", parse_mode='MarkdownV2', disable_notification=True)
    scan_and_send_files(best_proxy_url, "BestProxy", additional_text="`bestproxy.onecf.eu.org`", parse_mode='MarkdownV2', disable_notification=True)
